...
bord_H = np.array([champs_1[0, 1:-1]])
bord_D = np.array([champs_1[:-1, -1]]).T
bord_B = np.array([champs_1[-1, 1:]])
bord_G = np.array([champs_1[:, 0]]).T
start_time = time.time()
for i in range(0, 20000):

    Champs_Moy = (champs_1[2:, 1:-1] + champs_1[1:-1, 2:] + champs_1[:-2, 1:-1] + champs_1[1:-1, :-2])/4
    ...
    champs_1 = ScalarField(Transition_Matrice)
...
#ScalarField(teste).show()

#matrice_transitoire = np.concatenate(bord_H, résultat"""
